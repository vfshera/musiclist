...
<script>
    ...
    export default {
        ...
        methods:{
            validatePhone(num){
                const numReg = /254[71][0-9]{8}/

              return  (num.match(numReg)) ? true : false

            },
            donate(index){
                if(index == 1 && !this.validatePhone(this.phone)){

                    Swal.fire({
                        icon: 'error',
                        text: 'Ensure The Phonenumber Is As 254XXXXXXXXX'
                    })

                    return;
                }

                let finalAmount = this.gateways[index].multiplier * this.amount
                let info = (index == 0) ? ' Email '+this.email+'?' : (index == 1) ? ' Number +'+this.phone+'?' : ''
